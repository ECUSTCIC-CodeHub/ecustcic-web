...
│   ├── .vuepress/
│   │   └── config.js    # VuePress 配置（导航栏、侧边栏等）
│   ├── ecust-CourseShare/    # 课程攻略
│   ├── faq/                  # 网络 FAQ
│   ├── library/              # 图书数据库
│   ├── EcustMakerTurtorial/  # 游戏开发
│   └── quickstart/           # 维护指南
├── scripts/
...
启动后，技术文档可通过 `http://localhost:8081/tech/` 访问（VuePress 默认端口 8080，若主站已占用则自动递增）。
...
- `docs/.vuepress/config.js` — VuePress 配置（导航栏、侧边栏等）
